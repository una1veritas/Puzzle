...
class Board2p:
    ...
    def __eq__(self, another):
        ...
        if self.grids_per_player != another.grids_per_player :
            return False
        start_ix0 = self.get_player_start_index(0)
        end_ix0 = start_ix0 + self.grids_per_player
        start_ix1 = self.get_player_start_index(1)
        end_ix1 = start_ix1 + self.grids_per_player
        if self.next_move_player == another.next_move_player :
            if self.data[start_ix0: end_ix0] != another.data[start_ix0: end_ix0] :
                return False
            if self.data[start_ix1: end_ix1] != another.data[start_ix1: end_ix1] :
                return False
        else:
            if self.data[start_ix0: end_ix0] != another.data[start_ix1: end_ix1] :
                return False
            if self.data[start_ix1: end_ix1] != another.data[start_ix0: end_ix0] :
                return False
        return True

    def __hash__(self):
        p_tuple = list()
        start_ix = self.get_player_start_index(self.next_move_player)
        p_tuple.append(tuple(self.data[start_ix: start_ix + self.grids_per_player]))
        start_ix = (start_ix + self.grids_per_player) % len(self.data)
        p_tuple.append(tuple(self.data[start_ix: start_ix + self.grids_per_player]))
        return hash(tuple(p_tuple))
    ...
    def move(self, index: int) -> bool:
        ...
        """
        if index < self.get_player_start_index(self.next_move_player) \
        or index >= self.get_player_start_index(self.next_move_player) + self.grids_per_player :
        ...
        self.next_move_player += 1
        self.next_move_player %= self.NUMBER_OF_PLAYERS
